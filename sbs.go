...
func processMessage(message string, logger zap.SugaredLogger, trackedCache *lru.Cache, updates chan<- *pb.PositionUpdate) error {
...
// StartSbsTracking reads, processes, and posts position updates from an SBS channel.
func StartSbsTracking(addr string, logger zap.SugaredLogger, trackedCache *lru.Cache, updates chan<- *pb.PositionUpdate) {
	if addr == "" {
	...
	for {
		...
		if err != nil {
			logger.Errorf("Error connecting to %s: %v", addr, err)
		...
		for {
			...
			err = processMessage(message, logger, trackedCache, updates)
			if err != nil {
